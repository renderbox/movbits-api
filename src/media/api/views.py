...
def _finalize_video(batch):
    """Set video to S3 CDN and record the HLS entry point once all files are uploaded."""
    entry_point = _pick_entry_point(batch)
    video = batch.video
    video.cdn = Video.CDNChoices.S3_MEDIA_BUCKET
    video.video_key = entry_point
    video.save(update_fields=["cdn", "video_key"])
...
def create_upload_batch(request):
    ...
    hls_dir = video.get_video_hls_path(version=video.version)
    file_objs = []
    for f in files_data:
...
def list_uploads(request):
    ...
    if video_uuid:
        qs = qs.filter(batch__video__uuid=video_uuid)
    ...
    files = [
        {
            ...
            "video_uuid": str(f.batch.video.uuid),
        }
        for f in qs
    ]
